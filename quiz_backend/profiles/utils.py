...
def activate_user(activation_key):
    ...
    if SHA1_RE.search(activation_key):
        try:
            ...
            return False
        if profile.user.is_active and not profile.activation_key_expired():
...
def send_activation_email(user, site):
    ...
    The activation email will ma    print(request.data['username'])
    print(serializer.is_valid())ke use of two templates:
...
def re_activate(email):
    activated = RegistrationProfile.objects.get(user__email=email)
    activated.activation_key = create_activation_key(activated.user)
    activated.save()
    send_activation_email(activated.user, Site.objects.get_current())